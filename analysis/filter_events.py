...
def filter_rechits(file_path, out_path, run_number, hot_strips, bad_chambers, off_bx):
    ...
    v_region = ROOT.std.vector('i')()
    v_layer = ROOT.std.vector('i')()
    v_chamber = ROOT.std.vector('i')()
    v_ieta = ROOT.std.vector('i')()
    v_first_strip = ROOT.std.vector('i')()
    v_cluster_size = ROOT.std.vector('i')()
    v_scale = ROOT.std.vector('f')()
    ...
    for event in tqdm(rec_hits, total=tot_events):
        v_region.clear(); v_layer.clear(); v_chamber.clear(); v_ieta.clear(); v_first_strip.clear(); v_cluster_size.clear(); v_scale.clear();
        ...
        vec_lst = [event.region, event.layer, event.chamber, event.ieta,
                   event.first_strip, event.cluster_size]
        for region, layer, chamber, ieta, first_strip, cluster_size in zip(vec_lst):
            if chamber_err:
                region_str = "M" if region == -1 else "P"
                h_bad_chamber[f"{region_str}L{layer}"].Fill(chamber, inst_lumi)
                if bunch_id not in off_bx:
                    h_bad_chamber[f"{region_str}L{layer}_on"].Fill(chamber, inst_lumi)
                else:
                    h_bad_chamber[f"{region_str}L{layer}_off"].Fill(chamber, inst_lumi)
                continue
