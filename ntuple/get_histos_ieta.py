...
def fill(input_file, bad_eta_dict, h, chamber_events, fill_number):
    ...
    for entry in tqdm(tree, total=nHits):
        ...
        if fill_number == 8220:
            ...
            if lumi < 14000 or lumi > 21500: continue  
        elif fill_number == 10649:
            if lumi < 10000 or lumi > 18000: continue  
        elif fill_number == 10665:
            if lumi < 9500 or lumi > 21500: continue  
        elif fill_number == 10645:
            if lumi < 8000 or lumi > 11000: continue  
        else: 
            print('no lumi range provided')
            print(f"{fill_number = }")
            exit()
        ...
        for vec in zip(*vec_lst):
            ...
            if bad_eta_dict[gem_label][chamber-1][ieta-1]: continue
...
def draw(h, bad_eta_dict, file_path, fill_number):
    ...
    out_file = ROOT.TFile.Open(f"{file_path}/histos_95_ieta.root", "RECREATE")
    # out_file = ROOT.TFile.Open(f"{file_path}/histos_ieta.root", "RECREATE")
    for region in [-1, 1]:
        region_str = 'M' if region == -1 else 'P'
        for layer in [1, 2]:
    ...
    for chamber in ['even', 'odd']:
        for region in [-1, 1]:
            ...
            for layer in [1, 2]:
                ...
                inactivated_chamber = bad_eta_dict[f'GE11-{region_str}-L{layer}'][::2].sum(axis=0) if chamber == 'odd' else bad_eta_dict[f'GE11-{region_str}-L{layer}'][1::2].sum(axis=0)

                activated_chamber = np.array([18]*8) - inactivated_chamber
                ...
                for ieta, c in zip(range(1, 9), colors):
                    ...
                    h[f'{region}_{layer}_{ieta}_{chamber}'].Scale(1 / activated_chamber[ieta-1])
...
if __name__ == "__main__":
    ...
    if fill_number == 8220:
        ...
        bad_chamber["GE11-M-L2"][35] = True
        bad_chamber["GE11-P-L1"][4] = True
    elif fill_number == 10645:
        bad_chamber["GE11-M-L1"][14] = True
    elif fill_number == 10649:
        bad_chamber["GE11-M-L1"][14] = True

    effcut_wp = 95
    with open(f"/users/jheo/GEM-BKG-Analysis/analysis/txt/dqm_eff_{fill_number}_ieta.yaml", "r") as f:
        eff_dct_run = yaml.load(f, Loader=yaml.CLoader)
    avg_eff_chamber_dict = {}
    for region in ["M", "P"]:
        for layer in [1, 2]:
            save_key = f"GE11-{region}-L{layer}"
            avg_eff_chamber_dict[save_key] = np.zeros((36, 8))
    if fill_number == 10649:
        len_run = len(run_lst) - 2 # low stat. run 392359, 392362
    elif fill_number == 10645: # low stat. run 392293, 392294
        len_run = len(run_lst) - 2
    else: len_run = len(run_lst)
    for run in run_lst:
        if run == 392359: continue
        elif run == 392362: continue
        elif run == 392293: continue
        elif run == 392294: continue
        eff_dict = eff_dct_run[run]
        for region in ["M", "P"]:
            for layer in [1, 2]:
                eff_key = f"{region}-L{layer}"
                save_key = f"GE11-{region}-L{layer}"
                avg_eff_chamber_dict[save_key] += np.array([eff_dict[eff_key][i] for i in range(1, 37)]) / len_run
    ...
    for region in ["M", "P"]:
        for layer in [1, 2]:
            save_key = f"GE11-{region}-L{layer}"
            bad_eta_dict[save_key] = avg_eff_chamber_dict[save_key]*100 < effcut_wp

    for key in bad_chamber: # mask ieta partitions in bad chambers
        for i, flag in enumerate(bad_chamber[key]):
            if flag: 
                bad_eta_dict[key][i] = np.array([True]*8)

    for chamber in ['even', 'odd']:
        for region in [-1, 1]:
            region_str = 'M' if region == -1 else 'P'
            for layer in [1, 2]:
                key = f'GE11-{region_str}-L{layer}'
                inactivated_chamber = bad_eta_dict[key][::2].sum(axis=0) if chamber == 'odd' else bad_eta_dict[key][1::2].sum(axis=0)
                activated_chamber = np.array([18]*8) - inactivated_chamber
                print(f"{key}-{chamber}: {activated_chamber}")

    for run in run_lst:
        fill(f'{file_path}/out_{run}.root', bad_eta_dict, h, chamber_events, fill_number)
    draw(h, bad_eta_dict, file_path, f'{fill_number}')
